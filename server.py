...
_yt_cookie_file_cache = None


def _resolve_yt_cookie_file():
    global _yt_cookie_file_cache
    explicit = os.getenv('YTDLP_COOKIE_FILE', '').strip()
    if explicit:
        return explicit
    ...
    if _yt_cookie_file_cache and os.path.exists(_yt_cookie_file_cache):
        return _yt_cookie_file_cache

    try:
        raw = base64.b64decode(encoded).decode('utf-8')
        fd, path = tempfile.mkstemp(prefix='aura_yt_', suffix='.cookies.txt')
        with os.fdopen(fd, 'w', encoding='utf-8') as handle:
            handle.write(raw)
        _yt_cookie_file_cache = path
        return path
    except Exception as exc:
        app.logger.warning("Failed to decode YTDLP_COOKIES_B64: %s", exc)
        return ''
...
def audio_stream():
    ...
    def pick_playable_format(info_dict):
    ...
    extractor_client_sets = [
        ['android'],
        ['android', 'mweb'],
        ['mweb', 'android'],
        ['android', 'web'],
    ]
    cookie_file = _resolve_yt_cookie_file()
    last_error = None

    try:
        for clients in extractor_client_sets:
            ydl_opts = {
                'quiet': True,
                'skip_download': True,
                'noplaylist': True,
            ...
            try:
                ...
                chosen = pick_playable_format(info)
                if not chosen:
                    continue
                return jsonify({
                    'id': video_id,
                ...
                last_error = exc
                app.logger.warning(
                    "audio_stream extraction failed for %s with clients=%s: %s",
                    video_id,
                    clients,
                    exc
                )
        ...
        if last_error:
            return jsonify({'error': f'Audio stream temporarily unavailable: {last_error}'}), 503
